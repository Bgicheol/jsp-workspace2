...
import chap08_2.model.Order_List;
...
public class DispatcherServlet extends HttpServlet {
	...
	private Map<String, Order_List> processMapping = new HashMap<>();
	...
	public void init(ServletConfig config) throws ServletException {
		...
		processMapping.put(contextPath + "/pizza/order::GET", new Order_List());
	}
	@Override
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (nextPage.startsWith("redirect:")) {
				...
				request.getRequestDispatcher("/WEB-INF/quiz" + nextPage + ".jsp").forward(request, response);
			}
